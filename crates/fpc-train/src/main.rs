...
//!   X.bin      (n * FEAT_DIM_REL)        perspective-relative features
//!   Y.bin      (n * 4)                   per-row terminal reward, ABSOLUTE
//!                                        colour order (final shares)
//!   movers.bin (n * u8)                  side to move per row (to rotate
...
fn play_and_record(
    ...
    (xs, movers, score_shares(&st.scores), len)
}
...
fn main() -> std::io::Result<()> {
    ...
    let games: Vec<(Vec<f32>, Vec<u8>, [f32; 4], u32)> = (0..num_games)
        .into_par_iter()
        .map(|g| {
    ...
    for (gx, gm, label, len) in &games {
        xs.extend_from_slice(gx);
        movers.extend_from_slice(gm);
        for _ in 0..*len {
    ...
    std::fs::write(format!("{out_dir}/movers.bin"), &movers)?;
